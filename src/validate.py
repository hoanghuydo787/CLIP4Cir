...
def compute_adidas_val_metrics(relative_val_dataset: FashionIQDataset, clip_model: CLIP, index_features: torch.tensor,
                            index_names: List[str], combining_function: callable) -> Tuple[float, float]:
    """
    Compute validation metrics on Adidas dataset
    :param relative_val_dataset: Adidas validation dataset in relative mode
    :param clip_model: CLIP model
    :param index_features: validation index features
    :param index_names: validation index names
    :param combining_function: function which takes as input (image_features, text_features) and outputs the combined
    ...
    # Generate predictions
    predicted_features, target_names = generate_adidas_val_predictions(clip_model, relative_val_dataset,
                                                                    combining_function, index_names, index_features)

    print(f"Compute Adidas {relative_val_dataset.dress_types} validation metrics")
...
def generate_adidas_val_predictions(clip_model: CLIP, relative_val_dataset: FashionIQDataset,
        ...
        Tuple[torch.tensor, List[str]]:
    """
    Compute Adidas predictions on the validation set
    :param clip_model: CLIP model
    :param relative_val_dataset: Adidas validation dataset in relative mode
    :param combining_function: function which takes as input (image_features, text_features) and outputs the combined
                            features
    ...
    :return: predicted features and target names
    """
    print(f"Compute Adidas {relative_val_dataset.dress_types} validation predictions")
...
def adidas_val_retrieval(dress_type: str, combining_function: callable, clip_model: CLIP, preprocess: callable):
    """
    Perform retrieval on Adidas validation set computing the metrics. To combine the features the `combining_function`
    is used
    :param dress_type: Adidas category on which perform the retrieval
    :param combining_function:function which takes as input (image_features, text_features) and outputs the combined
                            features
    :param clip_model: CLIP model
    :param preprocess: preprocess pipeline
    ...
    return compute_adidas_val_metrics(relative_val_dataset, clip_model, index_features, index_names,
                                   combining_function)
...
def main():
    ...
    parser.add_argument("--dataset", type=str, required=True, help="should be either 'CIRR' or 'fashionIQ' or 'adidas'")
    ...
    if args.dataset.lower() == 'cirr':
        ...
        general_recallat10, general_recallat50 = adidas_val_retrieval('general', combining_function, clip_model,
                                                                     preprocess)
        average_recall10_list.append(general_recallat10)
        average_recall50_list.append(general_recallat50)

        print(f"\n{general_recallat10 = }")
        print(f"{general_recallat50 = }")
